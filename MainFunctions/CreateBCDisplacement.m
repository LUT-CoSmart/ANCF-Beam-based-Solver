function Body = CreateBCDisplacement(Body, Displacement, Boundary) % Creates boundary conditions
    ...
    FixedInd = feval(xlocName,DofsAtNode,NodalBC,Dofs);
    DisplInd = feval(xlocName,DofsAtNode,NodalDispl,DofsDispl);
    
    % Boundary Condition
    bc = true(TotalDofs,1); % Define vector of linear constraints  
    
    Applied_bc = zeros(TotalDofs,1);
