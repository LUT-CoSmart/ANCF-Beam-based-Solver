...
% ########## Problem's data ###############################################
Material=1; % Materials. Neo-Hookean (1), 2 - and 5 - contants Mooney-Rivlin (2, 5), GOH (0), K.-S. (3).
...
for n = nmesh % Loop over all defined meshes: nmesh=[1 ....])    
    Dvec=[const,H,W,L/n]; % gathering physical and dimensional element data in one vector
    ...
    for i=1:steps
        ...
        for ii=1:imax    
            tic; 
            [K,ff,Fext] = Kt(q,q0,u,phim,F_applied,xloc,nx,nn,nl,MaterialName,Dvec,detF0,Gint,Nint,sol_acegen,Element,ElemDofs,DofsAtNode,PosDofs,h);
