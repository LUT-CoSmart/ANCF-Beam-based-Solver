          ...
          if ~isempty(OutcomeOnBody)
             ...
             for i = 1:size(Xi,1)  % loop over all points
                 ...
                 elseif ContactType == "NitscheLin"
                    ...
                    Sigma_nn = abs( Normal_cont' * (F_cont * Sigma_cont * F_cont' - F_targ * Sigma_targ * F_targ' ) * Normal_cont);
                          
                    lambda  = gap * Sigma_nn;

                    Ftarg_loc =  penalty * gap * Normal_targ + 2 * lambda * Normal_targ; 
                    Fcont_loc =  penalty * gap * Normal_cont + 2 * lambda * Normal_cont; 
                 else
