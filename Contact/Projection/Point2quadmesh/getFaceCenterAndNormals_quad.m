function [face_mean_nodes,face_normals]=getFaceCenterAndNormals_quad(faces,nodes)
% determines the incenter of a face and the normal direction to the face    
    face_mean_nodes=zeros(size(faces,1),3);
    face_normals=zeros(size(faces,1),3);

    for count_face=1:size(faces,1)
        current_nodes=nodes(faces(count_face,:),:);
        face_mean_nodes(count_face,:)=getQuadInCenter(current_nodes);

        %% TODO: may be there is sense to make averaging for two triangles 
