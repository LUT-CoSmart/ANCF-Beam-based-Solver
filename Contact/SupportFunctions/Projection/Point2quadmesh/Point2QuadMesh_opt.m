function [distances,project_pts,outside, projected_faces] = Point2QuadMesh_opt(inputs, pts)
    ...
    % Inside quad = inside ABC or inside ACD
    insideABC = point_in_triangle(proj, A, B, C);
    insideACD = point_in_triangle(proj, A, C, D);
    inside = insideABC | insideACD;

    function inside = point_in_triangle(proj,A,B,C)  
    ...
    if any(~inside)
        ...
        [Qcd, d2cd] = proj_to_seg(C(idx,:), D(idx,:), proj(idx,:));
        [Qda, d2da] = proj_to_seg(D(idx,:),A(idx,:),proj(idx,:));

        % Choose minimum-distance candidate per point
        % Compare three distances without loops:
        d2 = [d2ab, d2bc, d2cd, d2da];
        [~, k] = min(d2, [], 2);

        Qmin = Qab;                       % default
        sel  = (k == 2); Qmin(sel,:) = Qbc(sel,:);
        sel  = (k == 3); Qmin(sel,:) = Qcd(sel,:);
        sel  = (k == 4); Qmin(sel,:) = Qda(sel,:);
